...
export default function TodoFilter({ filter, onChange, categories, total, active }: Props) {
  ...
  return (
    <div className="space-y-3 rounded-2xl border border-slate-800 bg-slate-900 p-3 shadow-sm sm:border-0 sm:bg-transparent sm:p-0 sm:shadow-none">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div className="grid grid-cols-3 rounded-xl bg-slate-800 p-0.5">
          {STATUS_TABS.map(tab => (
            <button
              key={tab.value}
      ...
      <div className="grid grid-cols-1 gap-2 sm:grid-cols-[1fr_1fr_auto]">
        ...
        <select
          ...
          className="min-h-10 w-full rounded-xl border border-slate-700 bg-slate-800 px-3 text-sm text-slate-300 outline-none focus:border-indigo-500 sm:w-auto"
        >
          {SORT_OPTIONS.map(s => (
